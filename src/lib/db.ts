...
export function upsertVote(vote: {
  ...
  const db = getDb();
  const stmt = db.prepare(`
    INSERT INTO votes (menu_date, user_name, user_email, rating_overall, rating_starch, rating_vegan_protein, rating_veg, rating_protein_1, rating_protein_2, comment)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
